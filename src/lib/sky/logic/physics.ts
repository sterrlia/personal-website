        ...
        for (const neighborItem of nearbyItems) {
            ...
            if (dist <= constants.starLinkDistance) {
                const linkExists = state.starLinks.some(link => {
                    return (link.firstStarId === star.id && link.secondStarId === neighborStar.id)
                        || (link.secondStarId === star.id && link.firstStarId === neighborStar.id)
                });
...
export function updateStars(p5: p5) {
    ...
    for (const item of moving) {
        ...
        star.vx = p5.constrain(star.vx, constants.maxStarVelocity * -1, constants.maxStarVelocity);
        star.vy = p5.constrain(star.vy, constants.maxStarVelocity * -1, constants.maxStarVelocity);

        star.x += star.vx;
        star.y += star.vy;
        star.x = p5.constrain(star.x, star.r, state.windowData.width - item.star.r);
        star.y = p5.constrain(star.y, star.r, state.windowData.height - item.star.r)
...
export function updateStarLinks(p5: p5) {
    ...
    for (let i = state.starLinks.length - 1; i >= 0; i--) {
        ...
        if (!link.active && (millis > link.timestamp + constants.inactiveLinkLifetime)) {
