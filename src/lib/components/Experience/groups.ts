import { Level, type ChoosenLevelType, type Group, type GroupItem, type GroupItemDefinition, type GroupScheme } from "./types";
    ...
    let filteredScheme = level === "all"
        ...
        : Object.fromEntries(
            Object.entries(scheme)
                .map(([key, definitions]: [string, GroupItemDefinition[]]) => [
                    key,
                    definitions.filter((definition) => definition.level === level),
                ])
        );
